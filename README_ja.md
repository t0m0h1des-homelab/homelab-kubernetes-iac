...
* **モダンなProxmoxサポート:** **Proxmox VE 9+** に完全対応するため、最新の `bpg/proxmox` プロバイダを採用。レガシーなプロバイダで発生するAPI互換性の問題を解決済み。
...
* **宣言的な構成管理:**
    ...
    * **Ansible:** OS設定およびKubernetes (Kubeadm) のブートストラップを管理。
...
| **構成管理** | **Ansible** | エージェントレスで軽量。KubeadmのブートストラップやOSの初期設定に最適。 |
...
### 4\. Kubernetesの構築 (Ansible)

インベントリファイルを作成し、Playbookを実行。
...
## 🔜 ロードマップ
  ...
  * [ ] **ArgoCD** の導入によるGitOpsベースのアプリケーションデプロイ
